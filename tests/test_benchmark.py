"""Validación estructural del benchmark de movies — sin MongoDB ni Ollama."""
import sys
sys.path.insert(0, ".")
import json
...
def test_required_fields_present_and_typed(pairs):
    for p in pairs:
        ...
        # el filtro de find es un dict no vacío; el pipeline, una lista no vacía
        assert p["expected_mql"], f"empty expected_mql in {p['id']}"


def test_no_write_operators_in_references(pairs):
    # Toda query de referencia debe ser de solo lectura.
    for p in pairs:
        _check_no_writes(p["expected_mql"])  # lanza ValueError si hay un operador de escritura
...
def test_dev_test_split_ratio(pairs):
    ...
    ratio = n_test / n
    # Mantener aproximadamente un split 70/30 dev/test.
    assert 0.20 <= ratio <= 0.40, f"test ratio {ratio:.2f} outside 0.20-0.40"


def test_every_complexity_has_both_splits(pairs):
    # El conjunto de test debe ser representativo: cada complejidad aparece en test.
